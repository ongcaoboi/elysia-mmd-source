import { AnimationMixer, Object3D, Quaternion, Vector3, LoopOnce } from 'three'
import { CCDIKSolver } from 'three/examples/jsm/animation/CCDIKSolver.js'
import { MMDPhysics } from 'three/examples/jsm/animation/MMDPhysics.js'

// modified by Neon (https://github.com/eloraa)
...
var MMDAnimationHelper = (function () {
  ...
  function MMDAnimationHelper(params) {
    ...
    this.enabled = {
      animation: true,
      start: () => {
        this.animation.clipAction.play()
      },
      reset: () => {
        this.animation.clipAction.reset()
      },
      stop: () => {
        this.animation.clipAction.stop()
      },
      ik: true,
      grant: true,
      physics: true,
  ...
  MMDAnimationHelper.prototype = {
    ...
    _callOnStart: function () {
      if (this._onStart) this._onStart.call(this)
    },
    onReset: function (t) {
      return (this._onReset = t)
    },
    _callOnReset: function () {
      this.audioManager.reset(
        this.audioManager.delayTime + this.audioManager.audioDuration,
      )
      this.audio.stop()

      if (this._onReset) this._onReset.call(this)
    },
    onStop: function (t) {
      return (this._onStop = t)
    },
    _callOnStop: function () {
    ...
    _addMesh: function (mesh, params) {
      ...
      this._setupMeshAnimation(mesh, params.animation, params.loop)
    ...
    _setupMeshAnimation: function (mesh, animation, loop) {
      ...
      if (animation !== undefined) {
        ...
        this.animation.mixer = objects.mixer

        for (var i = 0, il = animations.length; i < il; i++) {
          objects.mixer.clipAction = objects.mixer
            .clipAction(animations[i])
            .play()

          if (!loop) objects.mixer.clipAction.setLoop(LoopOnce, 0)

          this.animation.clipAction = objects.mixer.clipAction
        }
